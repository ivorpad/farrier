...
```bash
farrier advise --dir . --author claude
farrier advise --dir . --author codex --sessions none          # codebase evidence only
farrier advise --dir . --author claude --since 14d             # exact-project Claude sessions from the past 14 days
farrier advise --dir . --author codex --since all              # explicit full-history opt-in
farrier advise --dir . --author claude --only guidance,hooks,mcp
farrier advise --dir . --author codex --model <name> --json
```

The author selects the reasoning CLI, session source, evidence provider, recommendation owner, and native artifact routes. Advice never mixes providers. Farrier infers it only when exactly one provider CLI is available. With both installed, omitting `--author` is an error before session discovery.
...
The interactive advice workflow starts with a visible **Author** picker and shows Claude/Codex counts for **past 7 days**, **past 14 days**, and **all history**. With both CLIs available, neither author is preselected and Analyze stays disabled until the user chooses. With one CLI available, Farrier selects it. Move to a visible setup control with Up/Down or Tab, then use Left/Right to change that control's value. In the report, Up/Down selects a recommendation and immediately shows the observed problem, expected value, strongest evidence, and exact artifact Farrier would create. PageUp/PageDown scrolls the full report. The visible action row contains **Create selected** and **Create all (N)**; Left/Right focuses an action and Enter activates it, so individual creation remains the default.

**Create all** coordinates every supported recommendation in the report. Farrier plans file recommendations and authors skill recommendations concurrently, with at most three backend jobs running at once. The author recorded in `report.author` owns every job. Model and reasoning settings for that author are reloaded when the batch starts, and a failure does not fall back to the other provider. Unsupported/manual routes such as unverified plugin installation are retained in the result as **skipped** with an explanation. Each recommendation shows queued/running progress followed by **planned**, **created**, **skipped**, **failed**, or **cancelled**; retry runs only failed/cancelled work and preserves successful work.
...
While batch planning/authoring runs, Ctrl+C or Command-Z requests cancellation through the batch's one abort signal, stops queued work, terminates running backend process groups, and waits for all jobs to settle. A cancellation arriving after the atomic file transaction begins does not interrupt it mid-commit; the transaction finishes or rolls back first. OpenTUI exposes Command as the `super` modifier, so the binding is `super+z`, never plain `z`. The host terminal must deliver an enhanced Super-modified key event (for example through the Kitty keyboard protocol); terminals that intercept Command-Z or cannot encode Super will not deliver it, and Ctrl+C remains the portable cancellation key. Headless advice remains report-only, progress stages go to stderr, and `--json` stdout remains valid machine-readable JSON.
...
farrier advise --dir . --only skills --author codex --json
```
...
Farrier delegates authoring to the selected provider's creator: Claude uses the pinned `anthropics/skills` **skill-creator**, and Codex uses its built-in `$skill-creator`. It validates each staged result before touching the project. New local skills are placed directly in provider-native roots; Farrier does not call `skills add` for them.

The wizard has a **Create** step (Stack → Skills → Create → Hooks → Learn → Review): describe the skill, choose one or two available authors, and choose the primary author when both are selected. The primary handles refine and inline eval.

- One Claude author creates a real `.claude/skills/<name>` tree.
- One Codex author creates a real `.agents/skills/<name>` tree.
- Two authors create independent real trees in both roots. Both legs must validate before either is placed.
- `--shared` with one author creates a real `.agents/skills/<name>` tree and `.claude/skills/<name> -> ../../.agents/skills/<name>`.

Farrier records exact local refs in `.farrier.json`: `./.claude/skills@<name>` and/or `./.agents/skills@<name>`, ordered Claude then Codex. Project-authored native and shared skills do not write `skills-lock.json`; that file remains owned by skills-CLI installs. A missing or invalid manifest does not block placement, but the result notes that the skill was not recorded.
...
Queue as many skills as you like with the visible **Queue another** action, then activate **Create**. Up to three authoring runs execute at once in isolated staging roots. Placement is serialized because successful requests can update `.farrier.json`. Each run is a full provider session, so expect minutes. Headless:

```bash
farrier skill new "Convert financial tables to markdown" --author claude --yes
farrier skill new "Mask PII in outgoing prompts" --author claude --author codex --yes
farrier skill new "Route queries to docs or the balance API" --author codex --name query-router --yes --json
farrier skill new "Log token costs as JSON" --author codex --no-llm --yes
farrier skill new "Shared release checklist" --author claude --shared --yes
farrier skill eval pii-masker --author claude --json
```

`--no-llm` still requires an explicit author. `--shared` accepts exactly one author and cannot be combined with eval. One explicit `--model` cannot be sent to two authors; configure `models.claude.skillCreation` and `models.codex.skillCreation` instead. Existing destinations block the whole reviewed placement. `--force` can replace regular trees and safe in-project links, retains backups, and still refuses special files, unsafe parents, or links outside the project. Authoring failures leave validated sibling output under `.farrier-staging/authoring/` and place nothing.

Compatibility flags remain in 0.4.x with one stderr warning per flag, also copied to JSON `notes`. Advice/eval `--backend`, advice `--targets`, and skill-new `--agents`, `--mode`, and `--no-install` are removed in 0.5.0. Legacy JSON aliases stay through schema version 1. Legacy `./skills@<name>` refs remain readable until a separate migration command exists.

When two independent copies succeed, Farrier can compare them and ask you to pick a winner. Both are staged at neutral paths and judged blind twice with candidate labels swapped. A winner is recommended only when both passes agree. A tie or cancelled confirmation leaves both real trees and the manifest unchanged. Applying either winner always produces the canonical shared topology under the winner's directory name: the reviewed content becomes the real `.agents` tree and Claude gets the exact relative link. Stale losing-name refs and paths are removed in the same transaction. Auto-apply keeps the deleted copy under `.farrier-staging/trash/`.
