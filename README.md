...
When `per-agent` creates both copies successfully, the results screen — both `farrier skill new`'s and the wizard's Done screen — offers `e` to run a read-only eval. Farrier asks a backend to compare `.claude/skills/<name>` and `.agents/skills/<name>` using the pinned Anthropic skill-creator's comparator/analyzer guidance, marks the recommendation, and then waits: enter keeps both, `c` or `x` picks Claude or Codex, and a second `y` confirmation is required before Farrier deletes the losing directory and replaces it with a relative symlink to the survivor. Headless uses the same safety shape: `farrier skill eval <name>` is read-only; deletion+symlink requires both `--apply-winner claude|codex` and `--delete-loser-and-link`.
