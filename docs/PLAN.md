...
4. **CLI surface: context-aware wizard + headless plan/apply.** Bare `farrier` opens the existing three-route launcher. Headless creation supports explicit or detected stacks, `--dry-run`, `--yes`, reviewed `--force`, offline `--no-skills`, and `--json`. Preview and apply share one creation-plan model. Existing lifecycle surfaces are create/detect, registry list, update, learn, doctor, advice, and skill new/eval. Doctor performs static harness checks; it does not run hooks or project tests.
...
6. **Hook catalog v1** (pack-selected): secret shield, verb runner, tool policy, protected-file write guard, quality judge, and stop judge. Bindings are generated only for selected agents and tested compatible events. PostToolUse feedback is non-destructive; blocking and Stop paths fail closed on malformed selected input, invalid configuration, timeout, or internal failure.
7. **Semantic judge: tiered and disabled by default.** The per-edit quality judge gives non-destructive feedback; the Stop judge can block serious findings when enabled. Models remain configurable through the existing Claude/Codex backend settings.
...
- Repository acceptance uses `just check` and `just konsistent`; package-level verification also runs `bun test`, `bunx tsc --noEmit`, and the colocated hook suite.
