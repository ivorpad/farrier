# farrier — TUI + headless CLI that forges agent-first project harnesses

## Context

Every new project (Python, TypeScript, Rails…) needs the same agents-first harness — hooks protecting `.env`, tool-policy bans, pinned skills, structure rules (konsistent), AGENTS.md, verification verbs — currently hand-assembled per repo. **farrier** (renamed from forge-cli; bare npm name verified available) scaffolds that harness for any stack, targeting Claude Code (`.claude/`), Codex (`.codex/`, AGENTS.md), and shared `.agents/` assets. First implementation step: rename `~/src/tries/2026-07-02-forge-cli` → `2026-07-02-farrier`.
...
- **konsistent.json v1 grammar** — reference `~/src/tries/2026-07-02-konsistent-python/konsistent.json` + `konsistent.schema.json` (27K). Same grammar for TS and Python.
