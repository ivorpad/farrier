...
1. Run Farrier's Claude policy through the shared advice orchestrator from the project root:
...
2. For one category, pass one of:
   ...
   The explicit legacy registry-only spelling is `farrier advise skills`.
...
- Farrier resolves the project, profiles dependencies and workflows, scopes sessions to the exact project directory, redacts locally, and sends bounded task episodes to the selected backend.
...
See `UPSTREAM.md` for the exact Anthropic commit, Apache-2.0 attribution, and SHA-256 hashes. The files under `upstream/` are an unchanged reference snapshot; Farrier's Claude policy encodes that decision framework while the shared orchestrator owns execution and validation.
